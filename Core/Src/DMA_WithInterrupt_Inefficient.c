...
//IRQ DMA1 event handler
void DMA1_Channel5_IRQHandler(void){ //Handle the interrupt of I2C1
	...
	if(n==length-1)
	{
		...
		DMA1_Channel5->CMAR = (uint32_t) &i2c_buff[n];
		DMA1_Channel5->CCR |= DMA_CCR_EN;
	}
	else if(n==length)
	{
		DMA1_Channel5->CCR &= ~DMA_CCR_EN;
		DMA1_Channel5->CMAR = (uint32_t) &i2c_buff[0];
...
void DMA_set(void)
{
		...
		DMA1_Channel5->CCR |= DMA_CCR_TCIE | DMA_CCR_PL  | DMA_CCR_CIRC ;
}
...
int main(void) {
	...
	NVIC_SetPriority(DMA1_Channel5_IRQn, 1);   // Set priority
